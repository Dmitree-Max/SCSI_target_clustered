					     ...
					     int *post_ret)
{
	...
	pr_warn("compare_and_write_post: in\n");
						 ...
						 int *post_ret)
{
	...
	*kv_key = cmd->t_bidi_data_sg->page_link + cmd->t_bidi_data_sg->offset;
	...
	pr_warn("compare_and_write_callback: compare SUCC\n");
...
miscompare:
	pr_warn("Target/%s: Reload flow works from file\n",
			dev->transport->name);
	pr_warn("Target/%s: Send MISCOMPARE check condition and sense I changed\n",
		dev->transport->name);
...
sbc_compare_and_write(struct se_cmd *cmd)
{
	...
	pr_warn("sbc_compare_and_write: in\n");
